...
class DensityProfile():
    ...
    def calc_kde(self, z_SOLV=None, z_POLY=None, solvent=None, clip=(0,600),cut=600, out_folder="", system="System", save=False):
        ...
        if save:
            ...
            df.to_csv(f"{out_folder}/kde_{system}_poly.csv",index=False)
            df = pd.DataFrame(kde2, columns=["z","vol_frac"])
            df.to_csv(f"{out_folder}/kde_{system}_solv.csv",index=False)
        return kde1, kde2
